...
class PriceHistory extends StatefulWidget {
  String id;
  PriceHistory(this.id, {Key? key}) : super(key: key);
...
class _PriceHistoryState extends State<PriceHistory> {
  ...
  void initState() {
    super.initState();
    var q =
        "SELECT * from rates where product_id=${widget.id} order by dated desc";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          centerTitle: true,
          title: const Text("History and Analysis"),
        ),
        body: noData
            ? const Center(child: Text("No Price History Found for This Product"))
            : loader
